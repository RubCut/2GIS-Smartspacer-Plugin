...
- 🚗 на автомоб отиле;
- 🚶 пешком;
- 🚍 общественным транспоротом

Все три два используют общий пункт назначения. Его можно найти по адресу через
...
- настройки у каждой Complication свои: пункт назначения и ключ хранятся
  отдельно для каждой;
- синхронизация API-ключа: новая Complication сразу получает последний
  сохранённый ключ, а новый ключ, введённый в любой Complication, становится
  дефолтным для следующих;
...
3. Вставьте API-ключ 2ГИС. Ключ синхронизируется между Complication: новая
   Complication сразу подставит последний сохранённый вами ключ, а новый ключ,
   введённый в любой Complication, перезапишет дефолтный. Уже настроенные
   Complication сохраняют собственный ключ.
...
- API-ключ, адрес, координаты и кэш ETA хранятся локально в
  `SharedPreferences` — отдельно для каждой Complication, плюс синхронизированный
  дефолтный ключ;
